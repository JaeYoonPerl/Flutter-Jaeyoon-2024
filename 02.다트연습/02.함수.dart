...
void main(){
  ...
  showTxt('이순신 시리즈 영화 주요출연배우들:${actorList(actors.toList())}');
  ...
  showTxt('조인성의 취미는 ${recommActor['조인성']?['취미']}다! 사는곳은 ${recommActor['조인성']?['사는곳']}다! 조인성의 자동차는 ${recommActor['조인성']?['자동차']??"정보없음"}이다!');

  // [ 다트문법 널세이프티 적용사례!! ]
  // -> Map형 데이터일 경우 내부 속성 불일치 등으로
  // null이 나올때 에러로 연결되므로 이런 경우에 
  // 다트는 필수적으로 널 세이프티 처리를 강요한다.
  // 물음표(?)하나는 null일 경우 "null"문자열 출력대체
  // 물은표두개(??)는 null 발생시 문자열 대체한 경우 
  // 커스터마이징 대체 문자열을 출력하게 해준다.
...
void japanShip(Function bomb){ // bomb - 전달 익명함수
